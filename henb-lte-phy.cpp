...
void
HenbLtePhy::StartTx(vector<UserEquipment*>* Dst_UEs, vector<PacketBurst*>* Tx_Packets)            //这里还需要进一步进行修改
{

	TxPowerAllocation( Dst_UEs);                    //进行功率配置，信号配置等

	std::cout << "HeNodeB " << GetDevice()->GetIDNetworkNode() << " 开始物理层传输" << std::endl;
	int m = 0;
...
void
HenbLtePhy::TxPowerAllocation( vector<UserEquipment*>* Dst_UEs)        //功率配置，确定每个资源块上所有用户的流功率分配矩阵
{
	if (LTE_BANDWIDTH == 10)
		SetTxPower(41);
